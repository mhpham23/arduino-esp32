...
BLEExtAdvertising *BLEDevice::m_bleAdvertising = nullptr;
#else
...
#if defined(CONFIG_BT_NIMBLE_ROLE_CENTRAL)
std::array<BLEClient *, NIMBLE_MAX_CONNECTIONS> BLEDevice::m_pClients{nullptr};
#endif

bool BLEDevice::m_initialized{false};
uint32_t BLEDevice::m_passkey{123456};
bool BLEDevice::m_synced{false};
ble_gap_event_listener BLEDevice::m_listener{};
std::vector<BLEAddress> BLEDevice::m_whiteList{};
uint8_t BLEDevice::m_ownAddrType{BLE_OWN_ADDR_PUBLIC};

#ifdef CONFIG_BTDM_BLE_SCAN_DUPL
uint16_t BLEDevice::m_scanDuplicateSize{CONFIG_BTDM_SCAN_DUPL_CACHE_SIZE};
uint8_t BLEDevice::m_scanFilterMode{CONFIG_BTDM_SCAN_DUPL_TYPE};
#endif
 ...
 */
BLEExtAdvertising *BLEDevice::getAdvertising() {
  if (m_bleAdvertising == nullptr) {
    m_bleAdvertising = new BLEExtAdvertising();
  }
 ...
 */
bool BLEDevice::startAdvertising(uint8_t instId, int duration, int maxEvents) {
 ...
 */
bool BLEDevice::stopAdvertising(uint8_t instId) {
 ...
 * @return True if advertising started successfully.
 */
bool BLEDevice::startAdvertising(uint32_t duration) {
 ...
 * @return True if advertising stopped successfully.
 */
bool BLEDevice::stopAdvertising() {
 ...
 * @details Must only be called before calling BLEDevice::init.
 */
...
BLEClient *BLEDevice::createClient() {
  return createClient(BLEAddress{});
}  // createClient

/**
 * @brief Creates a new client object, each client can connect to 1 peripheral device.
 * @param [in] peerAddress A peer address reference that is copied to the new client
 * object, allows for calling BLEClient::connect(bool) without a device or address parameter.
 * @return A pointer to the new client object, or nullptr on error.
 */
BLEClient *BLEDevice::createClient(const BLEAddress &peerAddress) {
  for (auto &clt : m_pClients) {
    if (clt == nullptr) {
      clt = new BLEClient(peerAddress);
      return clt;
    }
  }
 ...
 */
bool BLEDevice::deleteClient(BLEClient *pClient) {
  if (pClient == nullptr) {
    return false;
  }
 ...
 */
size_t BLEDevice::getCreatedClientCount() {
  size_t count = 0;
 ...
 */
BLEClient *BLEDevice::getClientByHandle(uint16_t connHandle) {
  for (const auto clt : m_pClients) {
 ...
 */
BLEClient *BLEDevice::getClientByPeerAddress(const BLEAddress &addr) {
  for (const auto clt : m_pClients) {
 ...
 */
BLEClient *BLEDevice::getDisconnectedClient() {
  for (const auto clt : m_pClients) {
 ...
 */
std::vector<BLEClient *> BLEDevice::getConnectedClients() {
 ...
 */
esp_power_level_t BLEDevice::getPowerLevel(esp_ble_power_type_t powerType) {
 ...
 * @return True if the power level was set successfully.
 */
bool BLEDevice::setPower(int8_t dbm, BLETxPowerType type) {
  ...
  } else if (type == BLETxPowerType::Advertise) {
    success = setPowerLevel(espPwr, ESP_BLE_PWR_TYPE_ADV);
  } else if (type == BLETxPowerType::Scan) {
    success = setPowerLevel(espPwr, ESP_BLE_PWR_TYPE_SCAN);
  } else if (type == BLETxPowerType::Connection) {
    success = setPowerLevel(espPwr, ESP_BLE_PWR_TYPE_DEFAULT);
  }
 ...
 */
int BLEDevice::getPower(BLETxPowerType type) {
#ifdef CONFIG_IDF_TARGET_ESP32P4
  return 0xFF;  // CONFIG_IDF_TARGET_ESP32P4 does not support esp_ble_tx_power_get
#else
  esp_ble_power_type_t espPwr = type == BLETxPowerType::Advertise ? ESP_BLE_PWR_TYPE_ADV
                                : type == BLETxPowerType::Scan    ? ESP_BLE_PWR_TYPE_SCAN
 ...
 * @returns True on success.
 */
bool BLEDevice::deleteBond(const BLEAddress &address) {
 ...
 */
bool BLEDevice::isBonded(const BLEAddress &address) {
  ble_addr_t peer_id_addrs[MYNEWT_VAL(BLE_STORE_MAX_BONDS)];
  int num_peers, rc;
  ...
  for (int i = 0; i < num_peers; i++) {
    BLEAddress storedAddr(peer_id_addrs[i]);
 ...
 * @returns BLEAddress of the found bonded peer or null address if not found.
 */
BLEAddress BLEDevice::getBondedAddress(int index) {
  ble_addr_t peer_id_addrs[MYNEWT_VAL(BLE_STORE_MAX_BONDS)];
  int num_peers, rc;
  rc = ble_store_util_bonded_peers(&peer_id_addrs[0], &num_peers, MYNEWT_VAL(BLE_STORE_MAX_BONDS));
  ...
  return BLEAddress(peer_id_addrs[index]);
}
#endif
 ...
 */
bool BLEDevice::onWhiteList(const BLEAddress &address) {
 ...
 */
bool BLEDevice::whiteListAdd(const BLEAddress &address) {
 ...
 * @returns True if successful.
 */
bool BLEDevice::whiteListRemove(const BLEAddress &address) {
 ...
 */
size_t BLEDevice::getWhiteListCount() {
 ...
 * @returns The BLEAddress at the whitelist index or null address if not found.
 */
 ...
 */
bool BLEDevice::init(const std::string &deviceName) {
  if (!m_initialized) {
...
/**
 * @brief Shutdown the BLE stack/controller.
 ...
 */
bool BLEDevice::deinit(bool clearAll) {
  int rc = 0;
  if (m_initialized) {
    rc = nimble_port_stop();
    if (rc == 0) {
...
#if defined(CONFIG_BT_NIMBLE_ROLE_PERIPHERAL)
    if (BLEDevice::m_pServer != nullptr) {
      delete BLEDevice::m_pServer;
      BLEDevice::m_pServer = nullptr;
    }
#endif

#if defined(CONFIG_BT_NIMBLE_ROLE_BROADCASTER)
    if (BLEDevice::m_bleAdvertising != nullptr) {
      delete BLEDevice::m_bleAdvertising;
      BLEDevice::m_bleAdvertising = nullptr;
    }
#endif

#if defined(CONFIG_BT_NIMBLE_ROLE_OBSERVER)
    if (BLEDevice::m_pScan != nullptr) {
      delete BLEDevice::m_pScan;
      BLEDevice::m_pScan = nullptr;
    }
#endif
 ...
 * @return A BLEAddress object with the currently used address, or a NULL address if not set.
 */
 ...
 */
bool BLEDevice::setOwnAddr(const BLEAddress &addr) {
 ...
 */
void BLEDevice::setSecurityAuth(uint8_t auth_req) {
  BLEDevice::setSecurityAuth(auth_req & BLE_SM_PAIR_AUTHREQ_BOND, auth_req & BLE_SM_PAIR_AUTHREQ_MITM, auth_req & BLE_SM_PAIR_AUTHREQ_SC);
}  // setSecurityAuth
 ...
 */
void BLEDevice::setSecurityIOCap(uint8_t iocap) {
 ...
 */
void BLEDevice::setSecurityInitKey(uint8_t initKey) {
 ...
 * * 0x08: BLE_SM_PAIR_KEY_DIST_LINK
 */
void BLEDevice::setSecurityRespKey(uint8_t respKey) {
 ...
 */
void BLEDevice::setSecurityPasskey(uint32_t passkey) {
  m_passkey = passkey;
}  // setSecurityPasskey
 ...
 */
uint32_t BLEDevice::getSecurityPasskey() {
  return m_passkey;
}  // getSecurityPasskey
