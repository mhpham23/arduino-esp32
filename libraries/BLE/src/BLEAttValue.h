...
template<typename T, typename = void, typename = void> struct Has_data_size : std::false_type {};

template<typename T> struct Has_data_size<T, decltype(void(std::declval<T &>().data())), decltype(void(std::declval<T &>().size()))> : std::true_type {};

/* Used to determine if the type passed to a template has a c_str() and length() method. */
template<typename T, typename = void, typename = void> struct Has_c_str_length : std::false_type {};

template<typename T> struct Has_c_str_length<T, decltype(void(std::declval<T &>().c_str())), decltype(void(std::declval<T &>().length()))> : std::true_type {};
...
class BLEAttValue {
  ...
  time_t m_timestamp{};
#endif
  void deepCopy(const BLEAttValue &source);
...
public:
  /**
     ...
     */
  BLEAttValue(uint16_t init_len = CONFIG_NIMBLE_CPP_ATT_VALUE_INIT_LENGTH, uint16_t max_len = BLE_ATT_ATTR_MAX_LEN);
  ...
  /**
     ...
     * @param[in] max_len The max size in bytes that the value can be.
     */
  BLEAttValue(const uint8_t *value, uint16_t len, uint16_t max_len = BLE_ATT_ATTR_MAX_LEN);
  ...
  /**
     ...
     * @param[in] max_len The max size in bytes that the value can be.
     */
  BLEAttValue(const char *value, uint16_t max_len = BLE_ATT_ATTR_MAX_LEN) : BLEAttValue((uint8_t *)value, (uint16_t)strlen(value), max_len) {}
  ...
  /**
     ...
     * @param[in] max_len The max size in bytes that the value can be.
     */
  BLEAttValue(std::initializer_list<uint8_t> list, uint16_t max_len = BLE_ATT_ATTR_MAX_LEN) : BLEAttValue(list.begin(), list.size(), max_len) {}
  ...
  /**
     ...
     * @param[in] max_len The max size in bytes that the value can be.
     */
  BLEAttValue(const std::string str, uint16_t max_len = BLE_ATT_ATTR_MAX_LEN)
    : BLEAttValue(reinterpret_cast<const uint8_t *>(&str[0]), str.length(), max_len) {}
  ...
  BLEAttValue(BLEAttValue &&source) {
  ...
  const BLEAttValue &getValue(time_t *timestamp = nullptr) const {
  ...
  /**
     ...
     * @returns A reference to the appended BLEAttValue.
     */
  BLEAttValue &append(const uint8_t *value, uint16_t len);
  ...
  /**
     ...
     */
  template<typename T>
  typename std::enable_if<!std::is_pointer<T>::value && !Has_c_str_length<T>::value && !Has_data_size<T>::value, bool>::type setValue(const T &v) {
  ...
  /**
     ...
     */
  template<typename T> typename std::enable_if<Has_c_str_length<T>::value && !Has_data_size<T>::value, bool>::type setValue(const T &s) {
  ...
  /**
     ...
     */
  template<typename T> typename std::enable_if<Has_data_size<T>::value, bool>::type setValue(const T &v) {
  ...
  /**
     ...
     */
  template<typename T> typename std::enable_if<!std::is_pointer<T>::value, bool>::type setValue(const T &s) {
  ...
  /** @brief Operator; Append another BLEAttValue. */
  BLEAttValue &operator+=(const BLEAttValue &source) {
    return append(source.data(), source.size());
  }

  /** @brief Operator; Set the value from a std::string source. */
  BLEAttValue &operator=(const std::string &source) {
    setValue(reinterpret_cast<const uint8_t *>(&source[0]), source.size());
    return *this;
  }

  /** @brief Move assignment operator */
  BLEAttValue &operator=(BLEAttValue &&source);

  /** @brief Copy assignment operator */
  BLEAttValue &operator=(const BLEAttValue &source);

  /** @brief Equality operator */
  bool operator==(const BLEAttValue &source) const {
