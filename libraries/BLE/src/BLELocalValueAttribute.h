...
class BLELocalValueAttribute : public BLELocalAttribute {
public:
  /**
  ...
  /**
     ...
     */
  BLEAttValue getValue(time_t *timestamp = nullptr) const {
    return m_value;
  }
...
protected:
  ...
  /**
     * @brief Construct a new BLELocalValueAttribute object.
     ...
     */
  BLELocalValueAttribute(const BLEUUID &uuid, uint16_t handle, uint16_t maxLen, uint16_t initLen = CONFIG_NIMBLE_CPP_ATT_VALUE_INIT_LENGTH)
    : BLELocalAttribute(uuid, handle), m_value(initLen, maxLen) {}

  /**
     * @brief Destroy the BLELocalValueAttribute object.
     */
  virtual ~BLELocalValueAttribute() = default;

  /**
     * @brief Callback function to support a read request.
     * @param [in] connInfo A reference to a BLEConnInfo instance containing the peer info.
     * @details This function is called by BLEServer when a read request is received.
     */
  virtual void readEvent(BLEConnInfo &connInfo) = 0;
  ...
  /**
     ...
     * @param [in] connInfo A reference to a BLEConnInfo instance containing the peer info.
     * @details This function is called by BLEServer when a write request is received.
     */
  virtual void writeEvent(const uint8_t *val, uint16_t len, BLEConnInfo &connInfo) = 0;

  /**
     * @brief Get a pointer to value of the attribute.
     * @return A pointer to the value of the attribute.
     * @details This function is used by BLEServer when handling read/write requests.
     */
  const BLEAttValue &getAttVal() const {
    return m_value;
  }
