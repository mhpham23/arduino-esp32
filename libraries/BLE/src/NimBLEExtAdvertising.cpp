 ...
 * @brief Constructor.
 */
BLEExtAdvertising::BLEExtAdvertising()
  : m_deleteCallbacks{false}, m_pCallbacks{&defaultCallbacks}, m_advStatus(CONFIG_BT_NIMBLE_MAX_EXT_ADV_INSTANCES + 1, false) {}
 ...
 */
bool BLEExtAdvertising::setInstanceData(uint8_t instId, BLEExtAdvertisement &adv) {
  ...
  int rc = ble_gap_ext_adv_configure(instId, &adv.m_params, NULL, (pServer != nullptr) ? BLEServer::handleGapEvent : BLEExtAdvertising::handleGapEvent, NULL);
#else
  int rc = ble_gap_ext_adv_configure(instId, &adv.m_params, NULL, BLEExtAdvertising::handleGapEvent, NULL);
#endif
 ...
 * @param [in] instId The extended advertisement instance ID to assign to this data.
 * @param [in] data A reference to a BLEExtAdvertisement that contains the data.
 */
 ...
 */
bool BLEExtAdvertising::removeInstance(uint8_t instId) {
  if (stop(instId)) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertising::removeAll() {
  if (stop()) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertising::stop(uint8_t instId) {
 ...
 */
void BLEExtAdvertising::setCallbacks(BLEExtAdvertisingCallbacks *pCallbacks, bool deleteCallbacks) {
 ...
 */
bool BLEExtAdvertising::isActive(uint8_t instId) {
 ...
 */
bool BLEExtAdvertising::isAdvertising() {
 ...
 */
int BLEExtAdvertising::handleGapEvent(ble_gap_event *event, void *arg) {
  (void)arg;
  BLEExtAdvertising *pAdv = BLEDevice::getAdvertising();
  ...
  switch (event->type) {
    ...
    {
      pAdv->m_pCallbacks->onScanRequest(pAdv, event->scan_req_rcvd.instance, BLEAddress(event->scan_req_rcvd.scan_addr));
      break;
 ...
 * * BLE_HCI_LE_PHY_CODED
 */
BLEExtAdvertisement::BLEExtAdvertisement(uint8_t priPhy, uint8_t secPhy) {
  m_params.own_addr_type = BLEDevice::m_ownAddrType;
  m_params.primary_phy = priPhy;
  m_params.secondary_phy = secPhy;
  m_params.tx_power = BLEDevice::getPower(BLETxPowerType::Advertise);
}  // BLEExtAdvertisement
 ...
 */
void BLEExtAdvertisement::setLegacyAdvertising(bool enable) {
 ...
 */
void BLEExtAdvertisement::setScannable(bool enable) {
 ...
 */
void BLEExtAdvertisement::setConnectable(bool enable) {
#if defined(CONFIG_BT_NIMBLE_ROLE_PERIPHERAL)
 ...
 */
void BLEExtAdvertisement::setAddress(const BLEAddress &addr) {
  m_advAddress = addr;
 ...
 */
void BLEExtAdvertisement::setPrimaryChannels(bool ch37, bool ch38, bool ch39) {
 ...
 */
void BLEExtAdvertisement::setScanFilter(bool scanRequestWhitelistOnly, bool connectWhitelistOnly) {
 ...
 */
void BLEExtAdvertisement::setDirectedPeer(const BLEAddress &addr) {
  m_params.peer = *addr.getBase();
}  // setDirectedPeer

/**
 * @brief Enable or disable direct advertisements to the peer set with `BLEExtAdvertisement::setDirectedPeer`
 * @param [in] enable true = send directed advertisements to peer.
 * @param [in] high_duty true = use fast advertising rate, default - true.
 */
void BLEExtAdvertisement::setDirected(bool enable, bool high_duty) {
 ...
 */
void BLEExtAdvertisement::setMinInterval(uint32_t mininterval) {
 ...
 */
void BLEExtAdvertisement::setMaxInterval(uint32_t maxinterval) {
 ...
 * * BLE_HCI_LE_PHY_CODED
 */
void BLEExtAdvertisement::setPrimaryPhy(uint8_t phy) {
 ...
 * * BLE_HCI_LE_PHY_CODED
 */
void BLEExtAdvertisement::setSecondaryPhy(uint8_t phy) {
 ...
 */
void BLEExtAdvertisement::setAnonymous(bool enable) {
 ...
 */
void BLEExtAdvertisement::enableScanRequestCallback(bool enable) {
 ...
 */
void BLEExtAdvertisement::clearData() {
 ...
 */
bool BLEExtAdvertisement::setData(const uint8_t *data, size_t length) {
 ...
 * @return True if successful, false if the data is too large.
 */
bool BLEExtAdvertisement::addData(const uint8_t *data, size_t length) {
 ...
 * @return True if successful, false if the data is too large.
 */
bool BLEExtAdvertisement::addData(const std::string &data) {
 ...
 */
bool BLEExtAdvertisement::setAppearance(uint16_t appearance) {
  if (appearance == 0) {
 ...
 * * BLE_HS_ADV_F_BREDR_UNSUP - must always use with BLE
 * @return True if successful.
 * @details If the flag value is 0 then it will be removed from the advertisement if set previously.
 */
bool BLEExtAdvertisement::setFlags(uint8_t flag) {
  if (flag == 0) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setManufacturerData(const uint8_t *data, size_t length) {
  uint8_t header[2];
  header[0] = length + 1;
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setManufacturerData(const std::string &data) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setManufacturerData(const std::vector<uint8_t> &data) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setURI(const std::string &uri) {
  uint8_t header[2];
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setName(const std::string &name, bool isComplete) {
  uint8_t header[2];
 ...
 */
bool BLEExtAdvertisement::addServiceUUID(const BLEUUID &serviceUUID) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::addServiceUUID(const char *serviceUUID) {
 ...
 */
bool BLEExtAdvertisement::removeServiceUUID(const BLEUUID &serviceUUID) {
  uint8_t bytes = serviceUUID.bitSize() / 8;
  int type;
  switch (bytes) {
    case 2:  type = BLE_HS_ADV_TYPE_COMP_UUIDS16; break;
 ...
 * @return True if successful or uuid not found, false if uuid error or data could not be reset.
 */
bool BLEExtAdvertisement::removeServiceUUID(const char *serviceUUID) {
 ...
 */
bool BLEExtAdvertisement::removeServices() {
  return true;
}  // removeServices
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setCompleteServices(const BLEUUID &uuid) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setCompleteServices16(const std::vector<BLEUUID> &v_uuid) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setCompleteServices32(const std::vector<BLEUUID> &v_uuid) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setPartialServices(const BLEUUID &uuid) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setPartialServices16(const std::vector<BLEUUID> &v_uuid) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setPartialServices32(const std::vector<BLEUUID> &v_uuid) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setServices(bool complete, uint8_t size, const std::vector<BLEUUID> &uuids) {
  uint8_t header[2];
 ...
 */
bool BLEExtAdvertisement::setServiceData(const BLEUUID &uuid, const uint8_t *data, size_t length) {
 ...
 * @note If data length is 0 the service data will not be advertised.
 */
bool BLEExtAdvertisement::setServiceData(const BLEUUID &uuid, const std::string &data) {
 ...
 * @note If data length is 0 the service data will not be advertised.
 */
bool BLEExtAdvertisement::setServiceData(const BLEUUID &uuid, const std::vector<uint8_t> &data) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setShortName(const std::string &name) {
 ...
 * @return True if successful.
 */
bool BLEExtAdvertisement::setPreferredParams(uint16_t minInterval, uint16_t maxInterval) {
 ...
 */
bool BLEExtAdvertisement::addTxPower() {
  if (m_params.legacy_pdu) {
    ...
    data[2] = BLEDevice::getPower(BLETxPowerType::Advertise);
#else
    data[2] = 0;
#endif
    return addData(data, 3);
 ...
 */
int BLEExtAdvertisement::getDataLocation(uint8_t type) const {
  size_t index = 0;
 ...
 */
bool BLEExtAdvertisement::removeData(uint8_t type) {
  int dataLoc = getDataLocation(type);
  if (dataLoc != -1) {
 ...
 */
size_t BLEExtAdvertisement::getDataSize() const {
 ...
 */
std::string BLEExtAdvertisement::toString() const {
  std::string hexStr = BLEUtils::dataToHexString(&m_payload[0], m_payload.size());
  std::string str;
