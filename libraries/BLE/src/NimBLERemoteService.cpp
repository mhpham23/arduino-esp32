 ...
 */
BLERemoteService::BLERemoteService(BLEClient *pClient, const ble_gatt_svc *service)
  : BLEAttribute{service->uuid, service->start_handle}, m_pClient{pClient}, m_endHandle{service->end_handle} {}
 ...
 */
std::vector<BLERemoteCharacteristic *>::iterator BLERemoteService::begin() const {
 ...
 */
std::vector<BLERemoteCharacteristic *>::iterator BLERemoteService::end() const {
  return m_vChars.end();
}
 ...
 */
BLERemoteCharacteristic *BLERemoteService::getCharacteristic(const char *uuid) const {
 ...
 */
const std::vector<BLERemoteCharacteristic *> &BLERemoteService::getCharacteristics(bool refresh) const {
  if (refresh) {
    deleteCharacteristics();
...
int BLERemoteService::characteristicDiscCB(uint16_t conn_handle, const ble_gatt_error *error, const ble_gatt_chr *chr, void *arg) {
  ...
  if (error->status == 0) {
    pSvc->m_vChars.push_back(new BLERemoteCharacteristic(pSvc, chr));
    return 0;
  }
...
bool BLERemoteService::retrieveCharacteristics(const BLEUUID *uuidFilter) const {
  ...
  if (uuidFilter == nullptr) {
    rc = ble_gattc_disc_all_chrs(m_pClient->getConnHandle(), getHandle(), getEndHandle(), BLERemoteService::characteristicDiscCB, &taskData);
  } else {
    rc = ble_gattc_disc_chrs_by_uuid(
      m_pClient->getConnHandle(), getHandle(), getEndHandle(), uuidFilter->getBase(), BLERemoteService::characteristicDiscCB, &taskData
    );
  }
 ...
 */
BLEAttValue BLERemoteService::getValue(const BLEUUID &uuid) const {
 ...
 */
bool BLERemoteService::setValue(const BLEUUID &uuid, const BLEAttValue &value) const {
  const auto pChar = getCharacteristic(uuid);
  if (pChar) {
 ...
 */
void BLERemoteService::deleteCharacteristics() const {
  for (const auto &it : m_vChars) {
    delete it;
  }
  std::vector<BLERemoteCharacteristic *>{}.swap(m_vChars);
}  // deleteCharacteristics
 ...
 */
size_t BLERemoteService::deleteCharacteristic(const BLEUUID &uuid) const {
 ...
 */
std::string BLERemoteService::toString() const {
