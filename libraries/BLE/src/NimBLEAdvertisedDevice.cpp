 ...
 */
BLEAdvertisedDevice::BLEAdvertisedDevice(const ble_gap_event *event, uint8_t eventType)
#if CONFIG_BT_NIMBLE_EXT_ADV
  : m_address{event->ext_disc.addr}, m_advType{eventType}, m_rssi{event->ext_disc.rssi}, m_callbackSent{0}, m_advLength{event->ext_disc.length_data},
    m_isLegacyAdv{!!(event->ext_disc.props & BLE_HCI_ADV_LEGACY_MASK)}, m_sid{event->ext_disc.sid}, m_primPhy{event->ext_disc.prim_phy},
    m_secPhy{event->ext_disc.sec_phy}, m_periodicItvl{event->ext_disc.periodic_adv_itvl},
    m_payload(event->ext_disc.data, event->ext_disc.data + event->ext_disc.length_data) {
#else
  : m_address{event->disc.addr}, m_advType{eventType}, m_rssi{event->disc.rssi}, m_callbackSent{0}, m_advLength{event->disc.length_data},
 ...
 * @param [in] event The advertisement event data.
 */
void BLEAdvertisedDevice::update(const ble_gap_event *event, uint8_t eventType) {
#if CONFIG_BT_NIMBLE_EXT_ADV
 ...
 */
const BLEAddress &BLEAdvertisedDevice::getAddress() const {
  return m_address;
}  // getAddress
 ...
 */
uint8_t BLEAdvertisedDevice::getAdvType() const {
  return m_advType;
}  // getAdvType
 ...
 */
uint8_t BLEAdvertisedDevice::getAdvFlags() const {
  size_t data_loc;
 ...
 */
uint16_t BLEAdvertisedDevice::getAdvInterval() const {
  size_t data_loc;
 ...
 */
uint16_t BLEAdvertisedDevice::getMinInterval() const {
  size_t data_loc;
 ...
 */
uint16_t BLEAdvertisedDevice::getMaxInterval() const {
  size_t data_loc;
  if (findAdvField(BLE_HS_ADV_TYPE_SLAVE_ITVL_RANGE, 0, &data_loc) > 0) {
    const ble_hs_adv_field *field = reinterpret_cast<const ble_hs_adv_field *>(&m_payload[data_loc]);
    if (field->length == BLE_HS_ADV_SLAVE_ITVL_RANGE_LEN + 1) {
 ...
 */
std::string BLEAdvertisedDevice::getManufacturerData(uint8_t index) const {
 ...
 */
uint8_t BLEAdvertisedDevice::getManufacturerDataCount() const {
 ...
 * @return The URI data.
 */
std::string BLEAdvertisedDevice::getURI() const {
 ...
 */
std::string BLEAdvertisedDevice::getPayloadByType(uint16_t type, uint8_t index) const {
  size_t data_loc;
 ...
 */
BLEScan *BLEAdvertisedDevice::getScan() const {
 ...
 */
uint8_t BLEAdvertisedDevice::getTargetAddressCount() const {
 ...
 */
BLEAddress BLEAdvertisedDevice::getTargetAddress(uint8_t index) const {
  ...
  if (count > 0 && data_loc != ULONG_MAX) {
    ...
    if (field->length > index * BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN) {
      return BLEAddress{field->value + (index - 1) * BLE_HS_ADV_PUBLIC_TGT_ADDR_ENTRY_LEN, field->type};
    }
  }
 ...
 */
std::string BLEAdvertisedDevice::getServiceData(uint8_t index) const {
  uint8_t bytes;
 ...
 * @return The advertised service data or empty string if no data.
 */
std::string BLEAdvertisedDevice::getServiceData(const BLEUUID &uuid) const {
  uint8_t bytes;
  uint8_t index = 0;
  size_t data_loc = findServiceData(index, &bytes);
  ...
  while (data_loc < pl_size) {
    const ble_hs_adv_field *field = reinterpret_cast<const ble_hs_adv_field *>(&m_payload[data_loc]);
    if (bytes == uuid_bytes && BLEUUID(field->value, bytes) == uuid) {
      const char *field_data = reinterpret_cast<const char *>(field->value + bytes);
      return std::string(field_data, field->length - bytes - 1);
    }
 ...
 */
BLEUUID BLEAdvertisedDevice::getServiceDataUUID(uint8_t index) const {
  uint8_t bytes;
  size_t data_loc = findServiceData(index, &bytes);
  if (data_loc != ULONG_MAX) {
    const ble_hs_adv_field *field = reinterpret_cast<const ble_hs_adv_field *>(&m_payload[data_loc]);
    if (field->length >= bytes) {
      return BLEUUID(field->value, bytes);
    }
  }
 ...
 */
size_t BLEAdvertisedDevice::findServiceData(uint8_t index, uint8_t *bytes) const {
  *bytes = 0;
 ...
 */
uint8_t BLEAdvertisedDevice::getServiceDataCount() const {
 ...
 */
BLEUUID BLEAdvertisedDevice::getServiceUUID(uint8_t index) const {
  ...
  if (uuid_bytes > 0) {
    ...
    if (field->length > uuid_bytes * index) {
      return BLEUUID(field->value + uuid_bytes * (index - 1), uuid_bytes);
    }
  }
 ...
 */
uint8_t BLEAdvertisedDevice::getServiceUUIDCount() const {
 ...
 */
bool BLEAdvertisedDevice::haveConnParams() const {
 ...
 */
bool BLEAdvertisedDevice::haveAdvInterval() const {
 ...
 */
bool BLEAdvertisedDevice::haveURI() const {
 ...
 */
bool BLEAdvertisedDevice::haveType(uint16_t type) const {
 ...
 */
bool BLEAdvertisedDevice::haveTargetAddress() const {
 ...
 * @return The set ID.
 */
uint8_t BLEAdvertisedDevice::getSetId() const {
  return m_sid;
}  // getSetId
 ...
 */
uint8_t BLEAdvertisedDevice::getPrimaryPhy() const {
  return m_primPhy;
}  // getPrimaryPhy
 ...
 *  * BLE_HCI_LE_PHY_CODED
 */
uint8_t BLEAdvertisedDevice::getSecondaryPhy() const {
  return m_secPhy;
}  // getSecondaryPhy
...
uint8_t BLEAdvertisedDevice::findAdvField(uint8_t type, uint8_t index, size_t *data_loc) const {
 ...
 */
std::string BLEAdvertisedDevice::toString() const {
  ...
  if (haveManufacturerData()) {
    ...
    res += BLEUtils::dataToHexString(reinterpret_cast<const uint8_t *>(mfgData.data()), mfgData.length());
  }
 ...
 */
uint8_t BLEAdvertisedDevice::getAdvLength() const {
  return m_advLength;
}
 ...
 */
uint8_t BLEAdvertisedDevice::getAddressType() const {
 ...
 */
const std::vector<uint8_t> &BLEAdvertisedDevice::getPayload() const {
  return m_payload;
}
 ...
 */
const std::vector<uint8_t>::const_iterator BLEAdvertisedDevice::begin() const {
 ...
 */
const std::vector<uint8_t>::const_iterator BLEAdvertisedDevice::end() const {
