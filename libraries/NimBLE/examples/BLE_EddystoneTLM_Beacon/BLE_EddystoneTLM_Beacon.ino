...
void setBeacon() {
  ...
  Serial.printf("Random Temperature is: %d.%d 0x%04X\n", eddystoneTLM.getTemp() / 256, eddystoneTLM.getTemp() % 256 * 100 / 256);

  BLEAdvertisementData oAdvertisementData = BLEAdvertisementData();
  BLEAdvertisementData oScanResponseData = BLEAdvertisementData();
  oScanResponseData.setServiceData(BLEUUID("FEAA"), reinterpret_cast<const uint8_t *>(&eddystoneTLM.getData()), sizeof(BLEEddystoneTLM::BeaconData));
