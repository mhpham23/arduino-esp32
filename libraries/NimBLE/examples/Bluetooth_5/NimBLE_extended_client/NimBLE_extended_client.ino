...
class scanCallbacks : public BLEScanCallbacks {
  void onResult(const BLEAdvertisedDevice *advertisedDevice) override {
    ...
    if (advertisedDevice->isAdvertisingService(BLEUUID("ABCD"))) {
  ...
  void onScanEnd(const BLEScanResults &results, int rc) override {
...
void setup() {
  ...
  BLEDevice::init("BLE Extended Client");

  /** Create aBLE Scan instance and set the callbacks for scan events */
