...
static const uint8_t ble_base_uuid[] = {0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

/**
 * @brief Create a UUID from the native UUID.
 * @param [in] uuid The native UUID.
 */
BLEUUID::BLEUUID(const ble_uuid_any_t &uuid) : m_uuid{uuid} {}
...
BLEUUID::BLEUUID(const std::string &value) {
  if (value.length() == 4) {
    ...
    char *position;
    uint64_t first_half = (strtoull(&value[0], &position, 16) << 32) + (strtoull(position + 1, &position, 16) << 16) + strtoull(position + 1, &position, 16);
 ...
 */
const uint8_t *BLEUUID::getValue() const {
  switch (bitSize()) {
    case BLE_UUID_TYPE_16:  return reinterpret_cast<const uint8_t *>(&m_uuid.u16.value);
    case BLE_UUID_TYPE_32:  return reinterpret_cast<const uint8_t *>(&m_uuid.u32.value);
    case BLE_UUID_TYPE_128: return m_uuid.u128.value;
    default:                return nullptr;
  }
}  // getValue

/**
 * @brief Get a pointer to the BLE UUID base structure.
 * @return A Read-only pointer to the BLE UUID base structure.
 * @note The type value should be checked, if no 16, 32 or 128 then the UUID is not initialized.
 */
const ble_uuid_t *BLEUUID::getBase() const {
 ...
 */
BLEUUID BLEUUID::fromString(const std::string &uuid) {
  uint8_t start = 0;
 ...
 * @return The BLEUUID converted to 128bit.
 */
const BLEUUID &BLEUUID::to128() {
  // If we either don't have a value or are already a 128 bit UUID, nothing further to do.
  if (bitSize() != BLE_UUID_TYPE_128) {
    uint32_t val = bitSize() == BLE_UUID_TYPE_16 ? *reinterpret_cast<const uint16_t *>(getValue()) : *reinterpret_cast<const uint32_t *>(getValue());
 ...
 * @return The BLEUUID converted to 16bit if successful, otherwise the original uuid.
 */
const BLEUUID &BLEUUID::to16() {
  if (bitSize() == BLE_UUID_TYPE_128) {
 ...
 * @details This allows passing BLEUUID to functions
 * that accept std::string and/or or it's methods as a parameter.
 */
BLEUUID::operator std::string() const {
