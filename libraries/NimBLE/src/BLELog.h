...
#define NIMBLE_CPP_LOG_LEVEL_LOCAL(level, tag, format, ...)                                                                     \
  do {                                                                                                                          \
    if (level == ESP_LOG_ERROR) {                                                                                               \
      esp_log_write(ESP_LOG_ERROR, tag, NIMBLE_CPP_LOG_FORMAT(E, format), esp_log_timestamp(), tag __VA_OPT__(, ) __VA_ARGS__); \
    } else if (level == ESP_LOG_WARN) {                                                                                         \
      esp_log_write(ESP_LOG_WARN, tag, NIMBLE_CPP_LOG_FORMAT(W, format), esp_log_timestamp(), tag __VA_OPT__(, ) __VA_ARGS__);  \
    } else if (level == ESP_LOG_INFO) {                                                                                         \
      esp_log_write(ESP_LOG_INFO, tag, NIMBLE_CPP_LOG_FORMAT(I, format), esp_log_timestamp(), tag __VA_OPT__(, ) __VA_ARGS__);  \
    } else {                                                                                                                    \
      esp_log_write(ESP_LOG_DEBUG, tag, NIMBLE_CPP_LOG_FORMAT(D, format), esp_log_timestamp(), tag __VA_OPT__(, ) __VA_ARGS__); \
    }                                                                                                                           \
  } while (0)
