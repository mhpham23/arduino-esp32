...
/**
 * @brief Construct a default BLEHIDDevice object.
 ...
 */
void BLEHIDDevice::setPnp(uint8_t sig, uint16_t vid, uint16_t pid, uint16_t version) {
  uint8_t pnp[] = {
    sig,
 ...
 */
void BLEHIDDevice::setHidInfo(uint8_t country, uint8_t flags) {
 ...
 */
void BLEHIDDevice::setBatteryLevel(uint8_t level, bool notify) {
 ...
 */
BLECharacteristic *BLEHIDDevice::locateReportCharacteristicByIdAndType(uint8_t reportId, uint8_t reportType) {
  BLECharacteristic *candidate = m_hidSvc->getCharacteristic(inputReportChrUuid, 0);
  for (uint16_t i = 1; (candidate != nullptr) && (i != 0); i++) {
    BLEDescriptor *dsc = candidate->getDescriptorByUUID(featureReportDscUuid);
    BLEAttValue desc1_val_att = dsc->getValue();
 ...
 * @return BLECharacteristic* A pointer to the input report characteristic.
 ...
 * @return BLECharacteristic* A pointer to the output report characteristic.
 *                               Store this value to avoid computational overhead.
 * @details This will create the characteristic if not already created.
 */
BLECharacteristic *BLEHIDDevice::getOutputReport(uint8_t reportId) {
 ...
 * @return BLECharacteristic* A pointer to feature report characteristic.
 *                               Store this value to avoid computational overhead.
 * @details This will create the characteristic if not already created.
 */
BLECharacteristic *BLEHIDDevice::getFeatureReport(uint8_t reportId) {
 ...
 * @details This will create the characteristic if not already created.
 */
BLECharacteristic *BLEHIDDevice::getBootInput() {
  BLECharacteristic *bootInputChr = m_hidSvc->getCharacteristic(bootInputChrUuid);
  if (bootInputChr) {
    return bootInputChr;
  }
 ...
 * @details This will create the characteristic if not already created.
 */
BLECharacteristic *BLEHIDDevice::getBootOutput() {
  BLECharacteristic *bootOutputChr = m_hidSvc->getCharacteristic(bootOutputChrUuid);
  if (bootOutputChr) {
 ...
 */
BLECharacteristic *BLEHIDDevice::getHidControl() {
 ...
 */
BLECharacteristic *BLEHIDDevice::getProtocolMode() {
 ...
 */
BLECharacteristic *BLEHIDDevice::getBatteryLevel() {
 ...
 */
BLECharacteristic *BLEHIDDevice::getReportMap() {
 ...
 */
BLECharacteristic *BLEHIDDevice::getPnp() {
  return m_pnpChr;
}  // getPnp
 ...
 */
BLECharacteristic *BLEHIDDevice::getHidInfo() {
  return m_hidInfoChr;
}  // hidInfo
 ...
 */
BLEService *BLEHIDDevice::getDeviceInfoService() {
 ...
 */
BLEService *BLEHIDDevice::getHidService() {
  return m_hidSvc;
}  // getHidService
 ...
 */
BLEService *BLEHIDDevice::getBatteryService() {
  return m_batterySvc;
}  // getBatteryService
