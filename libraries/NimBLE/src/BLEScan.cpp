...
static BLEScanCallbacks defaultScanCallbacks;
...
BLEScan::BLEScan()
  : m_pScanCallbacks{&defaultScanCallbacks},
    ...
    m_scanParams{0, 0, BLE_HCI_SCAN_FILT_NO_WL, 0, 1, 1}, m_pTaskData{nullptr}, m_maxResults{0xFF} {}
  ...
  switch (event->type) {
    ...
    case BLE_GAP_EVENT_DISC:
    {
      ...
      if (advertisedDevice == nullptr) {
        ...
        if (pScan->m_maxResults > 0 && pScan->m_maxResults < 0xFF && (pScan->m_scanResults.m_deviceVec.size() >= pScan->m_maxResults)) {
          return 0;
        }
 ...
 */
void BLEScan::setDuplicateFilter(uint8_t enabled) {
 ...
 */
void BLEScan::setLimitedOnly(bool enabled) {
 ...
 */
void BLEScan::setFilterPolicy(uint8_t filter) {
 ...
 */
void BLEScan::setMaxResults(uint8_t maxResults) {
 ...
 */
void BLEScan::setScanCallbacks(BLEScanCallbacks *pScanCallbacks, bool wantDuplicates) {
 ...
 */
void BLEScan::setPhy(Phy phyMask) {
  m_phy = phyMask;
}  // setScanPhy
 ...
 */
void BLEScan::setPeriod(uint32_t periodMs) {
...
bool BLEScan::start(uint32_t duration, bool isContinue, bool restart) {
  ...
  int rc = ble_gap_ext_disc(
    ...
    m_period, m_scanParams.filter_duplicates, m_scanParams.filter_policy, m_scanParams.limited, m_phy & SCAN_1M ? &scan_params : NULL,
    m_phy & SCAN_CODED ? &scan_params : NULL, BLEScan::handleGapEvent, NULL
  );
#else
  int rc = ble_gap_disc(BLEDevice::m_ownAddrType, duration ? duration : BLE_HS_FOREVER, &m_scanParams, BLEScan::handleGapEvent, NULL);
#endif
  switch (rc) {
    case 0:
...
void BLEScan::clearResults() {
  if (m_scanResults.m_deviceVec.size()) {
    std::vector<BLEAdvertisedDevice *> vSwap{};
 ...
 */
int BLEScanResults::getCount() const {
 ...
 */
const BLEAdvertisedDevice *BLEScanResults::getDevice(uint32_t idx) const {
 ...
 */
std::vector<BLEAdvertisedDevice *>::const_iterator BLEScanResults::begin() const {
 ...
 */
std::vector<BLEAdvertisedDevice *>::const_iterator BLEScanResults::end() const {
 ...
 */
const BLEAdvertisedDevice *BLEScanResults::getDevice(const BLEAddress &address) const {
  for (const auto &dev : m_deviceVec) {
